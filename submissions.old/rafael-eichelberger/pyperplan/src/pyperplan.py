...
def search_plan(domain_file, problem_file, search, heuristic_class,
    ...
    i = 0
    nextInitialState = None
    finalSolution = []
    while i < len(problem.goals):
        problem.goal = problem.goals[i]
        task = _ground(problem)
        if nextInitialState is not None:
            task.initial_state = nextInitialState
        heuristic = None
        ...
        if solution is None:
            return None
        finalSolution.extend(solution)
        nextInitialState = heuristic.goals
        nextInitialState = solution[-1].add_effects
        i += 1
