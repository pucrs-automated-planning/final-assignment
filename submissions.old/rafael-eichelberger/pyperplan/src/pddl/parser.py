...
class ProblemDef(Visitable):
    ...
    def __init__(self, name, domainName, objects=None, init=None, goals=[]):
...
def parse_problem_def(iter):
    ...
    while not it.try_match(':end'):
        goals.append(parse_goal_stmt(it))
        it = next(iter)

    # assert end is reached
    iter.match_end()
    # create new ProblemDef instance
    return ProblemDef(probname, dom.name, objects, init, goals)
